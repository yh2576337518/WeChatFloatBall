...
typedef NS_ENUM(NSInteger, FloatShowStyle){
    ...
    FloatShowStyleShowContent//正在显示悬浮球里的内容，悬浮球aplha变成0
};
...
/**
 需要监控的类名
 */
@property (nonatomic, strong) NSMutableArray<NSString *> *monitorVCClass;
...
+(instancetype)shared{
    ...
    dispatch_once(&onceToken, ^{
        ...
        floatManager.monitorVCClass = [NSMutableArray array];
...
-(void)addFloatMonitorVCClasses:(NSArray<NSString *> *)VCClasses{
    for (NSString *classString in VCClasses) {
        if (![self.monitorVCClass containsObject:classString]) {
            [self.monitorVCClass addObject:classString];
        }
    }
}

-(id<UIViewControllerAnimatedTransitioning>)floatBallAnimationWithOperation:(UINavigationControllerOperation)operation fromViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC{
    ...
    if (operation == UINavigationControllerOperationPush) {
        ...
        if (fromVC == self.lastPopViewController) {
            ...
            //直接点击左上角返回
            if (!popWithPanGes && [self.monitorVCClass containsObject:NSStringFromClass([fromVC class])] && self.showStyle == FloatShowStyleShowContent) {
                animator = [self createAnimatorWithOperation:operation];
                vc = fromVC;
            }
        }
...
-(void)didShowViewController:(UIViewController *)viewController navigationController:(UINavigationController *)navigationController{
    if ([self.monitorVCClass containsObject:NSStringFromClass([viewController class])]) {
    ...
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if (notShowFloatContent) {
            ...
            if (point.x >= FloatScreenWidth / 6.0 && point.x <= FloatScreenWidth / 2.0) {
                ...
                [self.cancelFloatView showCancelFloatViewWithProgress:progress completion:nil];
            }
        ...
        if (notShowFloatContent) {
            [self.cancelFloatView showCancelFloatViewWithProgress:0 completion:nil];
        }
        ...
        if (fmax(velocityX, point.x) > FloatScreenWidth /2.0) {
            if (notShowFloatContent) {
                ...
                //正在显示悬浮球的内容
                //右滑手势拖动超过一半，手指离开屏幕，也会从当前触摸的位置缩小到悬浮球
                [animator replaceAnimation];
                [self p_animationWillEnd];
            }
...
-(void)dragFloatView:(UIPanGestureRecognizer *)ges{
    ...
    if (ges.state == UIGestureRecognizerStateBegan) {
        [self.cancelFloatView setCancelFloatViewShowing:YES];
        [self.cancelFloatView showCancelFloatViewWithProgress:1 completion:nil];
        ...
        //手势结束，处理右下角1/4圆
        [weakSelf.cancelFloatView  showCancelFloatViewWithProgress:0 completion:^{
            //圆消失后再将状态重置，防止在执行过程中让用户看见
            __weak __typeof(&*self)weakSelfs = self;
            if ([weakSelfs p_checkTouchPointInRound:inSuperViewPoint]) {
                [weakSelfs.cancelFloatView setCancelFloatViewShowing:NO];
            }
        }];
...
-(BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer{
    if ([NSObject currentNavigationController].viewControllers.count > 1) {
        if ([self.monitorVCClass containsObject:NSStringFromClass([[NSObject currentViewController] class])]) {
...
-(UIImageView *)floatView{
    if (!_floatView) {
        _floatView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"demo_link"]];
